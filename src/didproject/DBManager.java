...
//   private int max = 4; // Maximum number of entries
    int n;

    public void addLanguage(String languageName) {
        DBaccess.connect(host, port, user, password, dbname);
        DBaccess.update("INSERT INTO Lang (name,active) VALUES ('"+languageName+"',1)");
        DBaccess.disconnect();
    }
    
    public void updateLang(int id, String name, int active) {
        String q="UPDATE Lang SET name = '"+name+"', active = '"+active+"'  WHERE ID = '"+id+"'";
        DBaccess.connect(host, port, user, password, dbname);
        DBaccess.update(q);
	...
	public ArrayList<String> getIds(String tblName) {
     ...
     public boolean existsLang(String name) {
        ...
        try {
            DBaccess.connect(host, port, user, password, dbname);
            rs = DBaccess.retrieve("SELECT `ID` FROM `" + tblName + "` WHERE name='"+name+"'");
             if (!rs.first()) {
    ...
    public void createTable(String tblName) {
        ...
        String q = "CREATE TABLE \"" + tblName + "\""
                + "(" + fields[0] + " int(11) NOT NULL AUTO_INCREMENT, "
                + fields[1] + " VARCHAR(" + textWidth1 + ") NOT NULL, "
                + fields[2] + " int(1) NOT NULL, "
...
//    public boolean isFull(String tblName) {
//        return (size(tblName) == max);
//    }

    /**
     *  Returns the definition of a word in the dictionary.  Returns the definition.
     */
    public ArrayList<String> getLanguage(String id) {
        ...
        try {
            DBaccess.connect(host, port, user, password, dbname);
            rs = DBaccess.retrieve("SELECT * FROM `" + tblName + "` WHERE `ID` = '" + id + "'");
            ...
            System.err.println("ERROR: Get Languages e.getm: " + e.getMessage());
            System.err.println("SELECT * FROM `" + tblName + "` WHERE `ID` = '" + id + "'");
           }
         finally {
            DBaccess.disconnect();
             return result;
...
//    /**
//     *  Returns the maximum number of entries the dictionary can hold.
//     */
//    public int maxSize() {
//        return max;
//    }
